...
def similar_values_iter(v1, v2, e=1e-6):
	...
	#Olganix: max(abs(v1), abs(v2)) could be considered as 0.0 in some case, so make some division by 0.
    for v1, v2 in zip(v1, v2):
        if (v1 != v2) and (max(abs(v1), abs(v2)) > e) and ((abs(v1 - v2) / max(abs(v1), abs(v2))) > e):
            return False
    return True
