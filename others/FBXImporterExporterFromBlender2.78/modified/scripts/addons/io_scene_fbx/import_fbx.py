    ...
    def collect_armature_meshes(self):
        if self.is_armature:
            ...
            for m in meshes:
                ...
                #Olganix: do "m.parent = self" break the hierarchy. emdfbx and fbxemd now make hierarchy as near from emd structure. So we need to keep the hierarchy right.
                #m.parent = self
            self.meshes = meshes
        else:
            for child in self.children:
...
def load(operator, context, filepath="",
	...
	#Olganix: real_fps is always at -1.0, same if it's well specified into fbx. So, by default , we will use the 60 fps of the game and fbx animations.
    if real_fps <= 0.0:
        real_fps = 60.0
        #real_fps = 25.0
