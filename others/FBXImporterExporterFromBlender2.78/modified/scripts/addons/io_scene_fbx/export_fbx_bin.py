...
def fbx_data_mesh_elements(root, me_obj, scene_data, done_meshes):
    ...
    #Olganix: Blender make uniques names, but some names are relatives to material or other things. So we remove the part whitch make unique name
    a = me.name.split('.', 1 )
    geom.add_string(fbx_name_class(a[0].encode(), b"Geometry"))
...
def fbx_data_object_elements(root, ob_obj, scene_data):
    ...
    #Olganix: same problem on unique names.
    a = ob_obj.name.split('.', 1 )
    model.add_string(fbx_name_class( a[0].encode(), b"Model"))
