...
SkeletonDebug::SkeletonDebug(Ogre::Entity* entity, Ogre::SceneManager *man, Ogre::Camera *cam, float boneSize)
{
	...
	for (unsigned short int iBone = 0; iBone < numBones; ++iBone)
	{
		...
		ent = mSceneMan->createEntity("Repere.mesh");
		//tp = mEntity->attachObjectToBone(pBone->getName(), (Ogre::MovableObject*)ent);
		...
		/*
		Crash TODO see why (there is OgreOverlay_d.dll and good declaration in project properties).
...
void SkeletonDebug::showBones(string name, bool show)
{
	size_t nbBoneDebug = mListBoneDebug.size();
	for (size_t i = 0; i < nbBoneDebug; i++)
	{
	...
	for (it = mBoneEntities.begin(); it < mBoneEntities.end(); ++it)
	{
		if (((Ogre::Entity*)*it)->getName() != name)
			continue;
...
void SkeletonDebug::createBoneMesh()
{
	...
	if (mBoneMeshPtr.isNull())
	{
		...
		ColourValue col1 = ColourValue(0.6f, 0.6f, 0.6f, 1);
