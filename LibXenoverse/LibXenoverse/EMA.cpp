...
void EMA::writeEsk(ESK* esk)
{
	...
	//to secure that, we first build bones and after build the hierarchy.
	...
	for (size_t i = 0; i < nbElements; i++)
	{
		...
		if (!emoBone_ptr->parent)									//we take care only bone attached on root.
		{
			rootNode->mChildren.push_back(listTreeNode.at(i));
			listTreeNode.at(i)->mParent = rootNode;

			writeEsk__recursive(emoBone_ptr, bones, listTreeNode.at(i), listTreeNode);		//after the children will be done recursively.
		}
	}
...
|                             writeEsk__recursive				                 |
\-------------------------------------------------------------------------------*/
void EMA::writeEsk__recursive(EMO_Bone* emoBone, std::vector<EMO_Bone> &bones, EskTreeNode* treeNode, std::vector<EskTreeNode*> &listTreeNode)
{
	...
	while (emoBone_child != nullptr)
	{
		...
		if (isFound != (size_t)-1)					//if we found, we update the treeNode.
		{
