...
{ 
		/*
		//make holes in charac, first test for jamemba animation (but fixed)
		float3 position_scaled = position * float3(0.5,1,1) * 50.0;																	//we make a hole of 10 cm, so we have to multiply meters by 100.0
		if( (floor(position_scaled.x) + floor(position_scaled.y) ) % 2.0 == 0.0)
			clip(-1);
		*/
    ...
    float goldGlares = 0.0;
    if(color_traits.w>=0.0)
    	goldGlares = 1.0;
    ...
    //composition
    float3 color_final = colorDiffuse_base1.xyz * colorDiffuse_reducer + float3(1.0, 0.84313, 0.0) * goldGlares + colorSpecular.xyz * g_vParam3_PS.x + color_SamplerTemp0.xyz;	//colorReducer will make black stroke for toon/edges and damages. 
    ...
    if(g_bOutputGlareMRT_PS)
    	oColor1.xyz = (g_MaterialScale0_PS.xyz * g_MaterialScale0_PS.w) + g_vColor12_PS.xyz + float3(1.0, 0.84313, 0.0) * goldGlares;	//premultiplication of g_MaterialScale0_PS
}
